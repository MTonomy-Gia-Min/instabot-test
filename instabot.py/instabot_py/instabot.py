...
class InstaBot:
    ...
    def follow_these_users(self, usernames):
        while self.prog_run and self.login_status:
            now = datetime.datetime.now()
            # distance between start time and now
            dns = self.time_dist(
    ...
    def follow_by_username(self, usernames):
        for user in usernames:
            id = self.get_user_id_by_username(user)
            self.follow(user_id=id, username=user)


    def like_all_media_by_username(self, usernames):
        for user in usernames:
            url_info = self.url_user_detail % (user)
            info = self.s.get(url_info)
            print('info')
            ...
            if not json_info["entry_data"]["ProfilePage"][0]["graphql"]["user"]["followed_by_viewer"]:
                print('user name is ' + user)
                self.follow_by_username([user])

            for media in json_info["entry_data"]["ProfilePage"][0]["graphql"]["user"]["edge_owner_to_timeline_media"]["edges"]:
                media_id = media["node"]["id"]
                print(media_id)
                self.like(media_id)


    def dumb(self, usernames):
        for user in usernames:
            url_info = self.url_user_detail % (user)
            info = self.s.get(url_info)
            json_info = json.loads(
            ...
            user = json_info["entry_data"]["ProfilePage"][0]["graphql"]["user"]
            print("followed by viewer" + str(user["followed_by_viewer"]))
            print("blocked by viewer" + str(user["blocked_by_viewer"]))
            print("follows viewer" + str(user["follows_viewer"]))
